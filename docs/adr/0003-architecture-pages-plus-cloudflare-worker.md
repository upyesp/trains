...
The frontend is a statically-generated site hosted on **GitHub Pages** at the custom domain `www.viptrains.org.uk`. A single **Cloudflare Worker** acts as the data proxy: it holds the Real-Time Trains credentials, applies the 30s station-keyed cache, and serves the browser over CORS. **The browser never calls RTT directly.**
...
- The proxy serves a **separate origin** (e.g. `api.viptrains.org.uk` or a `*.workers.dev` URL) and must set **CORS** to allow the Pages origin only.
